...
import { flagConversationAsSpam, getConversationForMentor } from "@/lib/mentor/server";
...
export async function POST(_request: Request, { params }: RouteContext) {
  try {
    ...
    await flagConversationAsSpam(id, mentor.user.id);
    await publishConversationActivity(id);
    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("POST /api/mentor/dashboard/conversations/[id]/flag error:", error);
    return NextResponse.json({ error: "Không thể báo cáo spam." }, { status: 500 });
  }
}
